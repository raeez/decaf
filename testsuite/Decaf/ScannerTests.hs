...
-- plug in scanner here                      
scanner_ :: String -> String
scanner_ = scannerShow
...
singleTok = do
                ws
                t <- (operator <|> literal <|> identifier <|> terminal)
                ws
                return t

{-
scanTokens_ :: Parser [Token]
scanTokens_ = many singleTok
...
scanTokens :: String -> Report [DecafToken]
scanTokens input = RSuccess (map dToken $ eatFirst input)
...
numbers = TestList [
  TestLabel "decimal-0" ((scanTokens "0\n") ~?= RSuccess [DecLit "0"]),
  TestLabel "decimal-100" ((scanTokens "100") ~?= RSuccess [DecLit "100"]),
  TestLabel "hexadecimal-0" ((scanTokens "0x000") ~?= RSuccess [HexLit "000"]),
  TestLabel "hexadecimal-1" ((scanTokens "0x001") ~?= RSuccess [HexLit "001"]),
  TestLabel "hexadecimal-50" ((scanTokens "0x32") ~?= RSuccess [HexLit "32"]),
  TestLabel "dec10-hex16" ((scanTokens "10\r\r\n 0x10") ~?= RSuccess [DecLit "10", HexLit "10"]),
  TestLabel "enum 1...10" ((scanTokens "1     \t\t\t 2   \n\n\r\n   3 4 5\t6 0x7 0x8//haha, check out this comment ;-) xor'djuggajugga\n0x9//here we go again[0]@!\"!#@$!@#$!@%#$^%$^&%^&*^&*(\"\n0x1000") ~?= RSuccess [DecLit "1", DecLit "2", DecLit "3", DecLit "4", DecLit "5", DecLit "6", HexLit "7", HexLit "8", HexLit "9", HexLit "1000"]) 
  ]
...
strings = TestList [
  ...
  TestLabel "mixed-string" ((scanTokens "  \"  \\\"heLlo, I aM He\tre to sa\tve yoU!!\0!!\\\"\"") ~?= RSuccess [StrLit "  \"heLlo, I aM He\tre to sa\tve yoU!!\0!!\""])
  ]
...
characters = TestList [
  ...
  TestLabel "chained-characters" ((scanTokens "'.''A''B'") ~?= RSuccess [CharLit '.', CharLit 'A', CharLit 'B'])
  ]
