...
parse inputFile input = do
          case ps program input of
              RSuccess a -> putStrLn (show a) >> outputGraph a >> convertToPs >> exitSuccess
...
convertField (DecafVarField var@(DecafVar ty ident)) = Node (0, pretty_var var) Nothing
...
convertBlock (DecafBlock vars stms) = [Nil]
