...
data ConstantFact = ConstFactMap (M.Map ConstKey ConstData)
                  | ConstBot
                  deriving (Show, Eq)
...
constTransfer = mkFTransfer ft
 where
    ...
    ft :: LIRNode e x -> ConstFact -> Fact x ConstFact
    ft (LIRLabelNode {}) f         = ConstFactMap f
    ft (LIRRegAssignNode x (LIROperExpr (LIRIntOperand i))) f
                                   = ConstFactMap $ M.insert x i (varChanged f x) -- a op b -> x
    ...
    ft (LIRCallNode proc ret) f    = mkFactBase cseLattice [(proc, ConstFactMap f), (ret, ConstFactMap mkGlobalsTop)] -- remove all local vars (i.e. only keep global vars) when jumping to the function
      where
        mkGlobalsTop = M.mapWithKey globalToTop f
        globalToTop r i = if local x
                            then i
                            else ConstTop
    ...
    ft (LIRIfNode expr tl fl) f    = mkFactBase cseLattice [(tl, ConstFactMap f), (fl, ConstFactMap f)]  -- if expr the jmp tl else jmp fl
    ft (LIRJumpLabelNode l) f      = mkFactBase cseLattice [(l, ConstFactMap f)]        -- jmp l --> associate f with l 
...
constRewrite = mkFRewrite cp
 where
    ...
    cp node f = return $ liftM nodeToG $ s_node node
    cp node
      where
        ...
        rewriteRelExpr (LIRNotRelExpr a o)     = LIRNotRelExpr a (rewriteOperand o)
        rewriteRelExpr (LIROperRelExpr o)      = LIROperRelExpr (rewriteOperand o)

        rewriteOperand (LIRRegOperand r) = constReplace r
        rewriteOperand a = a

        constLook r =
          case M.lookup r f of
            Just (PElem i) -> trace ("rewriting register " ++ pp r ++ " with constant " ++ pp i) (LIRIntOperand i)
            Just Top       -> LIRRegOperand r
